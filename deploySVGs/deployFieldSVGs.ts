...
async function main() {
  ...
  if (!fs.existsSync(JSON_PATH) || fs.readFileSync(JSON_PATH).length == 0) {
    fs.writeFileSync(JSON_PATH, JSON.stringify({ shapes: {}, colors: {} }, null, 2))
  }

  // batch deploy contracts
  await batchDeploySVGs('Color', 1, deployer, networkName)
  await batchDeploySVGs('Shape', 1, deployer, networkName)
